    ...
    stages{        
        ...
        stage('Terraform init') {
            steps{                
                ...
                sh 'terraform init -no-color -inuput=false -upgrade=true -backend=true -force-copy -backend-config=bucket=adis-s3bucket -backend-config=workspace_key_prefix=terraform -backend-config=ley=test.tfstate'
                sh 'pwd'
            }
        }
